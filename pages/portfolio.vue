	...
	<div class="container mx-auto px-4 pt-20">
		...
		<section class="py-16 text-center">
			<div class="max-w-3xl mx-auto space-y-6">
				<h1 class="text-4xl md:text-6xl font-bold">
					<span class="bg-gradient-to-r from-foreground via-primary to-chart-1 bg-clip-text text-transparent"> My Portfolio </span>
				</h1>
				<p class="text-lg text-muted-foreground">Explore my mobile development projects, from innovative apps to complex systems</p>
			</div>
		</section>
		...
		<section class="py-16 text-center">
			<div class="max-w-2xl mx-auto space-y-6">
				...
				<p class="text-muted-foreground text-lg">Let's collaborate and bring your ideas to life with cutting-edge mobile solutions</p>
				<Button
					size="lg"
...
<script setup lang="ts">
	...
	const filteredProjects = computed(() => {
		...
		if (searchQuery.value) {
			...
			filtered = filtered.filter((p) => p.title.toLowerCase().includes(query) || p.description.toLowerCase().includes(query) || p.technologies.some((tech) => tech.toLowerCase().includes(query)));
		}
